...
class ContestAdminAPITest(APITestCase):
    ...
    def test_create_contest_with_tutor(self):
        data = copy.deepcopy(DEFAULT_CONTEST_DATA)
        data["contest_admin"] = [str(self.tutor1.id)]
    ...
    def test_illegal_update(self):
        ...
        data = copy.deepcopy(self.data)
        data.update(update_data)
        self.create_admin("test", "test123")
        response = self.client.put(self.url, data=data)
        self.assertTrue(response.data["data"].endswith("Illegal Manager"))
