...
from account.decorators import check_contest_permission, ensure_managed_by
...
class ContestAPI(APIView):
    ...
    def get(self, request):
        ...
        #找到admin所管理的所有Contest
        if request.user.is_admin():
            contests = contests.filter(contest_admin__in=str(request.user.id))
...
class ContestAnnouncementAPI(APIView):
    ...
    def put(self, request):
        ...
        try:
            ...
            ensure_managed_by(contest_announcement, request.user)
        ...
        if contest_announcement_id:
            if request.user.is_admin():
                ContestAnnouncement.objects.filter(id=contest_announcement_id,
                                                   contest__contest_admin__in=request.user.id).delete()
            else:
...
#查看提交数据 [admin(管理范围内), super_admin]
