...
class JudgeServerSafeSerializer(serializers.ModelSerializer):
    class Meta:
        model = JudgeServer
        exclude = ("ca_pem", "c_cert", "c_key")

class JudgeServerHeartbeatSerializer(serializers.Serializer):
    hostname = serializers.CharField(max_length=128)
    using_ports = serializers.ListField(child=serializers.IntegerField(min_value=0), allow_empty=True)
    available_ports = serializers.ListField(child=serializers.IntegerField(min_value=0), allow_empty=True)
    ...
    ca_pem = serializers.CharField(max_length=2600, allow_null=True)
    c_cert = serializers.CharField(max_length=2000, allow_null=True)
    c_key = serializers.CharField(max_length=4000, allow_null=True)
